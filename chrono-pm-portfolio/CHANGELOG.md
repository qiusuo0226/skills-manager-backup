...
## 3.30.3 — 2026-09-22（版本锁步）

> 与 Project 3.30.3 共用版本线。联邦根由 Project 升级程序按成员根执行。本包不编页、不手写成员正文。schema **0.17.0** 不变。回归 **1028**。
...
### 不做
- 集层编页；代更 Grok；本轮写入业务仓

### 收尾补记（2026-09-22，不另起版本）
- 用户指示收尾。锁步可投入使用。Grok 安装区不代更。集层不编页。业务仓未代迁。
- 分发包：`ChronoPM-Portfolio-Skill-v3.30.3.zip`（46）。

---

## 3.30.2 — 2026-09-21（版本锁步）

> 与 Project 3.30.2 共用版本线。手递写待办先快扫结转；已过后禁止开从头读结转/日报的代理。集层不编页。schema **0.17.0** 不变。回归 **1011**。

Blueprint Impact: metadata-only

### Changed
- 01 §2.2 步骤 4/8：快扫三问；已过同会话改点名文件
- SKILL 手递指针一句
...
## 3.30.1 — 2026-09-20（版本锁步）

> 与 Project 3.30.1 共用版本线。能力零改动、集层不编页。schema **0.17.0** 不变。回归 **997**。
...
- 用户指示收尾。锁步可投入使用。Grok 安装区不代更。集层不编页。

---

## 3.30.0 — 2026-09-20（版本锁步）

> 与 Project 3.30.0 共用版本线。能力零改动、集层不编页。schema **0.17.0** 不变。回归 **996**。
...
## 3.29.0 — 2026-09-13（版本锁步）

> 与 Project 3.29.0 共用版本线。投喂一次做完：抽出行即交接 SSOT；禁止下次再对包；禁止一个写任务塞多个成员项目；低置信只问归属。schema **0.17.0** 不变。回归 **980**。

Blueprint Impact: full

### Changed
- SKILL.md：投喂一次做完口径
- 01 §2.1/2.2：抽出行交接、按项目独立写任务、卡住切片 auto
- 02 V-14：禁止抽出后再打开 original 打分

### 不做
- 不手搓成员正文；不问项目内绑包（只问归属）

### 收尾（2026-09-13）
- 锁步 3.29.0。分发包 46 文件。Grok 不代更。

### 收尾补记（2026-09-14，不另起版本）
- 用户指示收尾。锁步可投入使用。Grok 安装区不代更。

---

## 3.28.0 — 2026-09-11（版本锁步）

> 与 Project 3.28.0 共用版本线。对外拍板同口径：拍板段禁止内部编号/结构版本/工序名；汇报段可以有版本号。schema **0.17.0** 不变。回归 **964**。

Blueprint Impact: metadata-only

### Changed
- SKILL.md §8 对外：与 Project 底线 19 同口径一句
- 06 §6：第 6 项对内名；对外分段
...
## 3.27.0 — 2026-09-10（版本锁步）

> 与 Project 3.27.0 共用版本线。V-15 范围聚合：只读各成员 `registers/scope-register.md`，缺表挂牌，不落数据源。schema **0.17.0**。回归 **961**。

Blueprint Impact: metadata-only

### Added
- V-15 范围聚合（02 §10a）：实时并合成员登记表；缺表挂牌；禁止 cache/事实源拷贝

### 收尾补记（2026-09-11，不另起版本）
- 与 Project 3.27.0 收尾同发。用户核验通过，可投入使用。Grok 不代更。audit 17/17。

---

## 3.26.0 — 2026-09-10（版本锁步）

> 与 Project 3.26.0 共用版本线。01 §2.1 补一句：成员根/纯单项目泛化材料由 Project 总闸处理；未知类别仍 ingest。触发列表与 §2 硬闸不改。schema 0.16.0。回归 **937**。
