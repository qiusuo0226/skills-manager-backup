...
材料投喂默认识别并落 `portfolio/reports/ingest/{batch}/`（原件+抽出行）与集层日志，禁止停在摘要、禁止问要不要落库。抽出行即交接 SSOT：本轮按行手递写完原文+能绑的包+已发生卡住，禁止「下次再对包」，禁止一个写任务塞多个成员项目。低置信只问归属（哪个成员项目）。高置信低/中风险同会话手递。细则 01 §2.1 / §2.2。
