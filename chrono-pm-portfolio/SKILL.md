...
材料投喂默认识别并落 `portfolio/reports/ingest/{batch}/`（原件+抽出行）与集层日志，禁止停在摘要、禁止问要不要落库。抽出行即交接 SSOT：本轮按行手递写完原文+能绑的包+已发生卡住，禁止「下次再对包」，禁止一个写任务塞多个成员项目。低置信只问归属（哪个成员项目）。高置信低/中风险同会话手递。细则 01 §2.1 / §2.2。写待办须先快扫结转（01 §2.2 步骤 4）；已过后禁止开从头读结转/日报的代理。
...
**对外（给集经理看）**：只说人话。禁止出现「建议更新清单」六字。说明有几件事需要点头、涉及哪些项目、选了会怎样；用编号选项（A/B/C）一次问完。内部表不对用户展示表头，除非对方问「改哪些文件」。拍板段禁止内部编号、结构版本、工序名，与 Project SKILL 底线 19 同口径；汇报段可以有版本号。
...
另：不得编造；不足须说明缺什么。推测必须标注。不得代集经理做资源/范围/里程碑决策。不得记录密码密钥 Token。不得在业务目录建 AI 管理文件。材料投喂禁止问「要不要落库 / 要不要记住」；写盘失败禁止谎报已存。投喂本轮必须按抽出行一次写完；禁止下次再对包；禁止一个写任务塞多个成员项目。
