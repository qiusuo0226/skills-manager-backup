...
- **何时加载**：命中拆文件/拆文档/入库源文档，或 SKILL.md「源文档拆解」行，**或** 10 §1.0 总闸通过的泛化可读文件。由 P-DOC-INGEST 强制 CALL，加载 `references/split-rules.md`。日报/待办/简单查询默认不加载。
- **产出**：`requirements/sources/{编号}/` 六件套 + 可选 `figures/`（抽出佐证图）。工人只写 `atoms/part-NN-*.md` 与抽出图文件，不写 ledger/_index。
