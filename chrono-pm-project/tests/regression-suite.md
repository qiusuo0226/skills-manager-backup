...
## 84. 相关性总闸 / 薄源（v3.26.0 CR-001）

施工只认合计 **937**（909+28）。阻断：REL-001、REL-006、REL-007、REL-011、REL-012。

| Case ID | Input | Expected | Type |
|---|---|---|---|
| REL-001 | 上传项目相关链接梳理 xlsx + 背景句，无记录/拆 | 薄源入库+回执；不问希望我怎么做 | positive |
| REL-002 | 仅背景句「对接政务服务网…两种静态页」 | journal；不对齐则不建 WP/REQ | positive |
| REL-003 | 「谢谢」/「哈哈」 | 不入库 | negative |
| REL-004 | 「这个需求怎么样了」 | 05，不入库 | regression |
| REL-005 | 「先别写」+材料 | 不入库 | regression |
| REL-006 | 工时 xlsx | 仍 01 §1.6，不进薄源 | regression |
| REL-007 | 会议转写 docx | WF-3，不进 sources | regression |
| REL-008 | 「把这份规格拆文件」 | 完整 P-SPLIT 可上提 REQ | regression |
| REL-009 | 明确他项目材料 | 不写本项目 | negative |
| REL-010 | 归属不清 | 只问归属，不问要做什么 | positive |
| REL-011 | 链接表薄源 | 不写 requirement-register | negative |
| REL-012 | REL-001 助手正文 | 无「您希望我怎么做」「需要我如何处理」「要不要记住」「要不要解析」 | negative |
| REL-013 | 泛化材料含里程碑改期 | 源可存；改期仍确认后写 | regression |
| REL-014 | 集根贴普通 xlsx | Project 不写子项目；交 Portfolio | regression |
| EX-019 | examples/19 新轮 | 有「只丢材料+背景句」演示 | positive |

## 85. 计划索引与上线路由（v3.26.0 CR-002）

阻断：PAN-001、PAN-002、PAN-004、PAN-005、PAN-007、PAN-013。
...
| PAN-006 | 用 superseded_by 表示互斥 | 违规 | negative |
| PAN-007 | 写入路径：索引缺行但 PLAN 文件在 | 补行，不删文件 | positive |
| PAN-008 | 看一下某计划 | 仍原样输出完整 PLAN | regression |
| PAN-009 | 把 index 当存在性：文件在、索引无就当没计划 | 违规 | negative |
| PAN-010 | 近义标题或 related_plans 只写一侧 | SUGGEST 一次补对端，不静默改旧 YAML | positive |
...
## 89. 对外拍板禁止技能黑话（v3.28.0 CR-20260911-001）

施工只认合计 **964**（961+3）。禁止沿用 961。阻断：RN-012、RN-013、RN-014。MIG-102 判定不变、Module 88 阻断清单不变；仅要求 P0 出现在汇报段。

| Case ID | Input | Expected | Type |
|---|---|---|---|
| RN-012 | 刚升完、范围草稿未认；「有什么需要我决策的吗」 | 上段可有版本；下段请你拍板用人话功能名/上不上；人能回 `1A` | positive |
| RN-013 | 同上，拍板段出现登记行号/PLAN 号/结构版本/回填状态/脚本名 | 本轮对外失败 | negative |
| RN-014 | 「明天待办」/待办清单 | 每一行仍有 TD 编号；1/2/3 不当主键；拍板禁编号不适用于本清单 | regression |

## 90. 投喂一次做完（v3.29.0 CR-20260913-001）

施工只认合计 **980**（964+16）。禁止沿用 964。阻断：ING-101、ING-102、ING-104、ING-111、ING-109。BS-018/019 预期不变（口述路径，本模块不改行）。

| Case ID | Input | Expected | Type |
|---|---|---|---|
| ING-101 | 集层一次混报+无表头表 | 一轮写完原文+能绑则绑；回执无绑包选择题 | positive |
| ING-102 | 「原文先记下，对包下次再说」 | 失败 | negative |
| ING-103 | 该人该项目唯一进行中 WP（投喂路径） | auto 绑 | positive |
| ING-104 | 投喂遇 0 个或多个进行中 WP | 原文+待归属；不问 A/B/C；不写待办缺工作包选择题；不经 Q-7 回落 WF-8 问绑 | negative |
| ING-105 | 话已是卡住 | 先记开着的问题；不问认不认 | positive |
| ING-106 | 可能以后才堵 | 不登风险 | regression |
| ING-107 | 同批同日同项目多人 | 本轮一次入队再合并 | positive |
| ING-108 | 只追加原文且当日已结转且文件齐全 | 走 ELSE，不重跑全员 Step 0 | positive |
| ING-111 | 应建档名单缺当天个人文件（即使本轮只追加原文） | 仍跑 Step 0；L42 仍阻断 inbox | positive |
| ING-109 | 一个写任务塞两个成员项目 | 失败 | negative |
| ING-110 | 抽出后再打开 original 打分 | 失败 | negative |

**16 §7 已有用例声明（CR-001，预期不变，不改行、不改合计）：**

| ID | 输入 | 本轮处置 | Type |
|---|---|---|---|
| BS-018 | 「给王五加个待办：调研竞品」（有人有截止无 WP） | 声明影响、预期不变：不落正式待办；问绑哪个已有 WP | 保留 |
| BS-019 | 两个 WP 均语义近似 | 声明影响、预期不变：必须追问归属，不得静默落库 | 保留 |

## 91. 源文档抽出图佐证（v3.29.0 CR-20260913-002）

阻断：FIG-001、FIG-005。

| Case ID | Input | Expected | Type |
|---|---|---|---|
| FIG-001 | 源 PDF 含关系图且能抽出 | `sources/{编号}/figures/FIG-*` 存在；ATOM/索引引用 | positive |
| FIG-002 | 抽不出像素 | 文字照拆；回执声明图在原件；不阻断 | negative |
| FIG-003 | 汇报引用该条款 | 可出示图或路径；拍板段仍无人话以外的内部号当问句 | positive |
