...
19. **拍板零黑话**（若本轮含请你拍板）：拍板段禁止内部编号/结构版本/工序名。待办清单仍给 TD 编号。细则见 SKILL 底线 19 与 `00-pm-main-rules.md` §5.0（本节不抄五句）。
...
每一次查询（同一会话第 2 问也一样，禁止「本会话刷过就不再比」）：
  IF 有 Python ≥3.9：
      ...
      源文档主题页过期另读 source_digest_status（独立字段，不并入 facts_fingerprint）
  ELSE（无 Python 或脚本失败）：
      不阻断、不报错当失败门
      不读过期 brain 当真理
...
跨切片范围/批次/纳入排除提问（「哪些（不）上线」、沿 PLAN→WP→功能点）：

1. 先出证据矩阵（事实 \| 来源路径 \| 位置）。信息点来自 `registers/scope-register.md` 命中行或 `relations` 显式边，不是语义切片。
2. 关联只读矩阵已有键/边。禁止新造因果或「其实是同一件事」。
3. 初步推导只消费上两步（范围类=筛 `纳入` 列）。能脚本筛的不口算。
4. 对照矩阵全表查漏/键冲突。修正只许补漏行、标待裁定、撤回无矩阵支撑的句子。

无矩阵 = 本轮失败，禁止对外结论。用户可见结论 + 矩阵（≤10 行，其余「共 N 行见表」），禁止思考过程。`回填-未确认` 必须标，不得当已确认。无表 → 挂牌未登记，禁止猜 PLAN 散文。
...
| 上线范围 / 批次 / 哪些（不）上线 / 十一窗口 | 「哪些对象本批次不上线」 | **有** `registers/scope-register.md` → 先读表（可经 `_index`），按批次/纳入筛行，走 §0.1 矩阵失败门；再最多打开 1 个 PLAN 核验指针。**无表** → 先 `plans/_index.md`（锚点），再最多 1～2 个命中 PLAN，并挂牌「范围未登记」。**禁止先读** risk-register。结论含归属 + PLAN/SR 号。缺 index：提示一次是否重建；未同意则读 PLAN 标题/§1 并标「锚点未填」，**禁止查询中全量扫描**（14 §2.1）。锚点为 — 不猜排除范围 |
...
7. **每一行必须写出待办编号** `TD-{缩写}-{YYYYMMDD}-{NNN}`，不得用 1、2、3 代替。缩写取当天 `_index` **§6 缩写映射**的现行缩写；映射没有则用花名册 §1 缩写列。00 号 CQ-4 的 1/2/3 只用于裁定选项。用户说「第 2 条」可按屏幕顺序认，但那一行仍必须有 TD 号。对外确认仍禁止甩文件路径/章节号；**待办编号是业务主键，查询必须给**。拍板段禁内部编号（SKILL 底线 19）**不适用于本清单**。
