...
| 3.27.0+ / schema 0.17.0 | 应有 `registers/` + `scope-register.md`（空表合法）+ `_index.md`。`.skill-version.json` 的 `scopeBackfillOpen` > 0 → **P0 升级未完成**（范围回填未清零）；范围推导必须横幅且不得把 `回填-未确认` 当已确认。日报/待办不阻断。项目内 `ai/scripts/` 若与包内 `refresh_views`/verify 功能重叠 → HINT 迁 `backup/`，禁止再跑；**不自动删**、不删 PLAN/WP/待办正文 |
...
健康报告属「结果汇报」段：可以有版本号、路径、脚本名。同一轮若还要人认草稿或清零，必须另起「请你拍板」段，拍板段禁止结构版本、登记行号、回填状态、脚本名。见 SKILL 底线 19 与 `00-pm-main-rules.md` §5.0。
...
**程序执行（v3.30.3）**：会话开始钩子运行 `scripts/enforce_workspace_upgrade.py`，超时 **120 秒**（只在本句规定这个数，钩子文件与它保持一致）。技能版本更高，或版本相同但已拆标准文件还没串成「切片 → 来源指针和文档链接 → 工作包编号」，都由这个程序处理。串不完不改该工作区的 `skillVersion`。模型禁止自己改这个字段。低版本技能仍不得改高版本工作区。

做不到每次都跑成：钩子超时或崩溃后会话仍会开始；项目文件夹未受信任时项目级钩子不跑；工具钩子拒绝改版本文件是尽力而为，宿主不支持或钩子失败时挡不住。这些情况下版本号保持原样。

**禁止**：跳过中间版本直接升级（如从 1.0.0 直接跳到 2.0.0）。模型不得自行改版本号。程序按升级文件逐级执行，不把没做完的存量说成可以投入使用。
