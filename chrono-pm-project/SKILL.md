...
17. **纠偏只写事实源或词库 §2**。禁止只改 brain / active-entities / index / 图 / `_digest`（主题页）。用户指出错误 = 已确认，不进八块。AI 自检冲突仍待确认。改源文档切片须同轮重编主题页。
18. **每个对外结论必须能指回证据**。无源结论 = 失败。brain 禁止待拍板节。汇报段可出示源文档抽出图或路径；拍板段仍走底线 19。
19. **对外拍板禁止技能黑话**：同一轮「结果汇报」和「请你拍板」必须分段。拍板段禁止内部编号、结构版本、工序名、脚本名、回填状态。发消息前五句自检过不了 = 本轮对外失败，先重写再发：①这段是问人认不认还是报进度；②问人的句子有没有内部编号/结构版本/工序名；③不看上文能不能回 `1A`；④证据路径等人问「改了哪些」再给（处理类落点回执除外）；⑤词典没有的新黑话先用人话解释。禁词只约束提问/拍板段；查询待办清单仍给待办编号。细则：`00-pm-main-rules.md` §5.0。
20. **投喂一次做完**：混报/进度表/拆文件本轮必须落下原文 + 能确定的待办映射 + 已发生卡住。禁止「下次再对包」。投喂/日报映射：该人该项目进行中且正常的工作包恰好 1 个则绑，否则标待归属、当场不问，**不经 Q-7 回落 WF-8 问绑**。WF-8「无上下文仍必须问绑哪个」只适用于口述直接创建（以及纪要正式行动项/派活等非投喂入口）。只问关问题/出组/删除/扩范围（外加口述绑包）。细则 01/04/00。
